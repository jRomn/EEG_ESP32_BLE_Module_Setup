...
// =============================
void setUp(void) {
    reset_adc_state();  // Cleans buffer, index, blink count, and filter
}

void tearDown(void) {
    // Called after each test
    // Optional cleanup (free memory, reset flags, etc.)
    // For example:
...
void test_buffer_fills_and_wraps(void) {
    ...
    // Arrange: Mock 3 sine samples (no real ADC)
    ...
    // Assert: Filled, wraps safe, no overflow
    ...
    TEST_ASSERT_EQUAL_INT(0, buffer_index);  // Wrapped to 0 again
}
